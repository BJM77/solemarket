...
export function CardCamera({ onCapture, isLoading }: CardCameraProps) {
    ...
    const processFile = async (file: File) => {
        ...
        try {
            ...
            // Fallback to capture if validation throws an unexpected error
            onCapture(file);
        }
    };
    ...
    if (validationError) {
        return (
            <div className="w-full aspect-[2.5/3.5] max-w-sm mx-auto border-2 border-red-500/20 bg-red-500/5 rounded-2xl flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in-95">
    ...
    return (
        <div
            className={cn(
                "relative flex flex-col items-center justify-center w-full aspect-[2.5/3.5] max-w-sm mx-auto border-2 border-dashed rounded-2xl transition-all overflow-hidden bg-slate-50",
                isDragging ? "border-indigo-500 bg-indigo-500/5" : "border-slate-200",
        ...
        >
            ...
            <div className="flex flex-col items-center gap-4 p-6 text-center w-full">
                ...
                <div>
                    <h3 className="text-xl font-black text-slate-900 tracking-tight">Scan Your Card</h3>
                ...
                <div className="w-full space-y-2 text-left bg-white/50 p-4 rounded-xl border border-slate-100">
                    ...
                    <div className="flex items-start gap-2 text-sm text-slate-700">
                        <span className="text-amber-500 font-bold">•</span>
                        <span><strong>Tap the screen</strong> to focus</span>
                    </div>
                    <div className="flex items-start gap-2 text-sm text-slate-700">
                        <span className="text-amber-500 font-bold">•</span>
                        <span>Fill the frame completely</span>
                    </div>
                </div>
                ...
                <div className="flex gap-3 w-full mt-2">
                    <Button 
                        ...
                        className="flex-1 h-12 bg-indigo-600 hover:bg-indigo-700 font-bold shadow-lg"
                    >
