    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                ...
                <Card
                    className={`cursor-pointer transition-all group hover:shadow-lg ${selectedType === 'collector-cards' ? 'border-indigo-600 ring-2 ring-indigo-600 ring-offset-2' : 'hover:border-indigo-500'}`}
                    onClick={() => onSelect('collector-cards')}
                >
                    <CardContent className="p-8 flex flex-col items-center text-center space-y-4">
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center transition-colors ${selectedType === 'collector-cards' ? 'bg-indigo-600' : 'bg-indigo-100 group-hover:bg-indigo-600'}`}>
                            <div className="w-12 h-16 border-2 border-indigo-600 bg-white/50 rounded flex items-center justify-center group-hover:border-white transition-colors">
                                <span className={cn("text-[10px] font-black", selectedType === 'collector-cards' ? "text-white" : "text-indigo-600 group-hover:text-white")}>PSA</span>
                            </div>
                        </div>
                        <div>
                            <h3 className="font-bold text-xl uppercase tracking-tight">Collector Cards</h3>
