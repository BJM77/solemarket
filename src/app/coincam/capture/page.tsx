...
import { syncService } from "@/coincam/lib/sync-service";
...
export default function CoinPhotoBooth() {
  ...
  const getDeviceTips = (device: DeviceProfile) => {
    ...
    if (device.manufacturer === 'google') {
      return "Pixel Macro Mode is active. Move closer for pristine coin detail.";
    }
    if (device.manufacturer === 'apple') {
      return "iOS macro capture active. Keep background dark to isolate coin edge.";
    }
    return "Keep coin flat, centered, and aligned with circle overlay.";
  };
  ...
  return (
    <div className="h-screen bg-black text-white font-mono flex flex-col overflow-hidden">
      <header className="p-3 bg-zinc-900 border-b border-white/10 flex justify-between items-center z-20">
        <Button variant="ghost" size="icon" onClick={() => router.push('/coincam')}><ArrowLeft className="w-5 h-5" /></Button>
              ...
              <div className="mt-6 space-y-4">
                {syncStatuses.size === 0 && syncQueue.length === 0 && (
                  <div className="text-center py-10 opacity-30">
                    ...
                    <p className="text-[8px] text-zinc-500 mt-1">Capture front/back of a coin to begin</p>
                  </div>
                )}
      ...
      <main className="flex-grow flex items-center justify-center relative">
        ...
        {sessionThumbnails.length > 0 && (
          <div className="absolute left-4 top-24 bottom-32 w-14 overflow-y-auto flex flex-col gap-2.5 scrollbar-none z-10">
            {sessionThumbnails.map((url, i) => (
              <div key={i} className="w-12 h-12 rounded-full border border-white/20 bg-zinc-900 overflow-hidden relative shrink-0 shadow-md">
        ...
        {/* Camera Viewport Framing Overlay - Coin Circle HUD */}
        <div className={cn(
          "w-72 h-72 md:w-80 md:h-80 border-2 rounded-full flex items-center justify-center relative transition-all duration-300 bg-white/[0.02]",
