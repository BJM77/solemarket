...
import { syncService } from "@/shoecam/lib/sync-service";
...
export default function ShoePhotoBooth() {
  ...
  // 4 steps sequence
  const [currentStep, setCurrentStep] = useState<'FRONT_45' | 'SIDE' | 'TOP' | 'LABEL'>('FRONT_45');
  ...
  const capturePhoto = async () => {
    ...
    try {
      ...
      // Keep aspect ratio 4:3 for sneaker crops
    ...
    return new Promise<QualityMetrics>((resolve) => {
      const analyze = () => {
        try {
          ...
          const quality = analyzeImageQuality(tempCanvas, currentStep === 'LABEL' ? { minBlur: 20 } : undefined);
          resolve(quality);
        } catch (err) {
  ...
  const getStepGuide = (step: typeof currentStep) => {
    switch (step) {
      case 'FRONT_45':
        return "45° Angle: Place shoe facing diagonally. Toe towards bottom-left, heel towards top-left.";
      case 'SIDE':
        return "Side Profile: Align shoe side with horizontal guideline.";
      case 'TOP':
        return "Top-Down View: Position shoe vertically showing laces, tongue and toe box.";
      case 'LABEL':
        return "Tag/Label: Locate size tag inside shoe tongue or inner wall. Focus closely.";
    }
  };
  ...
  return (
    <div className="h-screen bg-black text-white font-mono flex flex-col overflow-hidden">
      <header className="p-3 bg-zinc-900 border-b border-white/10 flex justify-between items-center z-20">
        <Button variant="ghost" size="icon" onClick={() => router.push('/shoecam')}><ArrowLeft className="w-5 h-5" /></Button>
        
        <div className="flex gap-2 items-center">
          <Badge className={cn("text-[9px] font-black uppercase tracking-widest", 
            currentStep === 'FRONT_45' ? "bg-primary text-black" : 
            currentStep === 'SIDE' ? "bg-orange-500 text-white" :
            currentStep === 'TOP' ? "bg-blue-500 text-white" :
          ...
          <Button 
            ...
            onClick={() => router.push('/shoecam/review')}
          >
      ...
      <div className="flex-1 relative flex items-center justify-center bg-zinc-950">
        ...
        {showHUD && (
          <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center p-6">
            {currentStep === 'FRONT_45' && (
              <div className="w-[85%] max-w-[400px] aspect-[4/3] border-2 border-dashed border-primary/60 rounded-xl relative">
                {/* Diagonal orientation indicator */}
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-full h-0.5 bg-primary/20 rotate-12 absolute" />
                  <span className="text-[9px] font-black text-primary bg-black/80 px-2 py-0.5 rounded uppercase tracking-wider">Toe facing Bottom-Left</span>
                </div>
              </div>
            )}
            {currentStep === 'SIDE' && (
              <div className="w-[90%] max-w-[420px] aspect-[16/9] border-2 border-dashed border-orange-500/60 rounded-xl relative">
                <div className="absolute inset-x-0 top-1/2 border-t border-orange-500/20" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[9px] font-black text-orange-400 bg-black/80 px-2 py-0.5 rounded uppercase tracking-wider">Align Side Profile</span>
                </div>
              </div>
            )}
            {currentStep === 'TOP' && (
              <div className="h-[80%] max-h-[380px] aspect-[9/16] border-2 border-dashed border-blue-500/60 rounded-xl relative">
                <div className="absolute inset-y-0 left-1/2 border-l border-blue-500/20" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[9px] font-black text-blue-400 bg-black/80 px-2 py-0.5 rounded uppercase tracking-wider rotate-90">Align Top-Down</span>
                </div>
              </div>
            )}
            {currentStep === 'LABEL' && (
              <div className="w-36 h-36 border-2 border-dashed border-purple-500/60 rounded-lg relative flex items-center justify-center">
                <Target className="w-6 h-6 text-purple-400 animate-pulse" />
                <span className="absolute bottom-2 text-[8px] font-black text-purple-400 bg-black/80 px-1.5 py-0.5 rounded uppercase tracking-widest">Focus Size Tag</span>
              </div>
            )}
          </div>
        )}
