  ...
  async processUpload(
    ...
    const status: SyncStatus = {
      ...
      steps: [
        ...
        { id: 'ai_identify', label: 'AI Coin Identification', status: 'pending' },
    ...
    try {
      ...
      const frontPath = `raw/coins/${id}_front.jpg`;
      ...
      if (upload.backBlob) {
        ...
        const backPath = `raw/coins/${id}_back.jpg`;
      ...
      try {
        ...
        await this.updateStep(status, 'ai_identify', 'error', `AI Failed: ${aiErr.message}. Storing coin raw.`, onStatusUpdate);
      }
      ...
      const docRef = doc(db, 'coin_imports', id);
      const resData = aiResult as any;
      const isComplete = !!(
        resData &&
        resData.coinName &&
        resData.denomination &&
        resData.year
      );
      ...
      await this.updateStep(status, 'firestore', 'success', `Created coin import doc: ${id}`, onStatusUpdate);
