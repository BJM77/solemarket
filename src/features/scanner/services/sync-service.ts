...
export interface GenericPendingUpload {
  id: string;
  frontBlob: Blob;
  backBlob?: Blob;
  createdAt: string;
...
export interface SyncConfig {
    storagePathPrefix: string; // e.g. 'raw/coins' or 'raw/cards'
    firestoreCollection: string; // e.g. 'coin_imports' or 'scans'
    aiIdentifyFn: (frontBase64: string, backBase64?: string) => Promise<any>;
    onCompleteStatus?: (aiResult: any) => string; // Function to determine final status e.g., 'VERIFIED' vs 'NEEDS_REVIEW'
    processResultData?: (aiResult: any) => any; // Function to map AI result to DB schema
}
  ...
  async processUpload(
    ...
    const status: SyncStatus = {
      ...
      steps: [
        ...
        { id: 'ai_identify', label: 'AI Identification', status: 'pending' },
    ...
    try {
      ...
      if (!user) {
        await this.updateStep(status, 'auth', 'success', 'Guest Session', onStatusUpdate);
      } else {
        await this.updateStep(status, 'auth', 'success', `Authenticated`, onStatusUpdate);
      }
      
      // Step 2: Upload Front Image
      await this.updateStep(status, 'upload_front', 'processing', 'Starting upload...', onStatusUpdate);
      
      const frontPath = `${config.storagePathPrefix}/${id}_front.jpg`;
      ...
      if (upload.backBlob) {
        ...
        const backPath = `${config.storagePathPrefix}/${id}_back.jpg`;
      ...
      try {
        ...
        aiResult = await config.aiIdentifyFn(frontImageBase64, backImageBase64);
        await this.updateStep(status, 'ai_identify', 'success', 'Item identified successfully!', onStatusUpdate);
      } catch (aiErr: any) {
        console.error('AI Identification error:', aiErr);
        await this.updateStep(status, 'ai_identify', 'error', `AI Failed: ${aiErr.message}. Storing raw.`, onStatusUpdate);
      }
      ...
      const docRef = doc(db, config.firestoreCollection, id);
      
      const processedAiResult = config.processResultData ? config.processResultData(aiResult) : aiResult;
      const finalStatus = config.onCompleteStatus ? config.onCompleteStatus(processedAiResult) : 'NEEDS_REVIEW';
      ...
      return { success: true, aiResult: processedAiResult as Record<string, any>, frontUrl, backUrl, docId: id };
      
    } catch (error: any) {
      console.error('[GenericSyncService] Error:', error);
...
export const syncService = new GenericSyncService();
