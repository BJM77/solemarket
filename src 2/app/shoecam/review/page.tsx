...
export default function ShoeReviewQueue() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this shoe capture?")) return;
    setActionId(id);
    try {
      await deleteDoc(doc(db, "shoe_imports", id));
      toast({ title: "Shoe Deleted", description: "Shoe capture queue item removed successfully." });
    } catch (err: any) {
      console.error(err);
      toast({ variant: "destructive", title: "Delete Failed", description: err.message || "Failed to delete shoe." });
    } finally {
      setActionId(null);
    }
  };

  const handleSubmit = async (item: ShoeImport) => {
    setActionId(item.id);
    try {
      ...
      toast({ title: "Shoe Submitted", description: "Successfully promoted and listed on the marketplace!" });
    } catch (err: any) {
      console.error(err);
      toast({ variant: "destructive", title: "Submit Failed", description: err.message || "Failed to submit shoe." });
    } finally {
      setActionId(null);
    }
  };
  ...
  const filtered = imports.filter(i => {
    ...
    const matchesSearch = 
      ...
      (i.styleCode?.toLowerCase().includes(term) || false) ||
    ...
    <div className="min-h-screen bg-black text-white">
      <header className="flex items-center justify-between p-4 bg-zinc-900 border-b border-white/10 sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push('/shoecam')} className="text-zinc-400 hover:text-white hover:bg-zinc-800">
            <ArrowLeft className="w-5 h-5" />
          </Button>
          <div>
            <h1 className="text-xl font-black uppercase font-headline tracking-tighter">Review Queue</h1>
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Sneaker Verification Workspace</p>
          </div>
        </div>
        <Badge className="bg-primary text-black font-black px-5 py-1 text-[10px] tracking-widest">{imports.length} SHOES</Badge>
      </header>
          ...
          {filtered.map((item: ShoeImport) => (
            <Card key={item.id} className="overflow-hidden group hover:border-primary/50 hover:shadow-glow transition-all bg-zinc-900 border-white/10 text-white">
               <div className="aspect-square relative bg-zinc-950 overflow-hidden rounded-t-xl">
                 <img src={item.front45ImagePath} className="w-full h-full object-cover transition-transform group-hover:scale-105 animate-fade-in" alt="front 45" />
               ...
               <CardContent className="p-5">
                  <div className="mb-4">
                    <h3 className="font-black text-xs truncate uppercase tracking-tight text-white">
                      {item.brand ? `${item.brand} ${item.model}`.trim() : 'Pending Identification'}
                    </h3>
                    <p className="text-[10px] font-bold text-zinc-500 mt-1 uppercase tracking-widest">
                      {item.styleCode || 'No SKU'} {item.sizeUs ? `• US ${item.sizeUs}` : ''}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/5 gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <Button 
                        ...
                        onClick={() => router.push(`/shoecam/review/${item.id}`)}
                        disabled={actionId === item.id}
                      >
          ...
          {filtered.length === 0 && !loading && (
            <div className="col-span-full py-24 text-center">
               <div className="bg-zinc-900 p-10 rounded-full inline-block mb-6 ring-8 ring-zinc-955">
                 <Footprints className="w-16 h-16 text-zinc-600" />
               </div>
               ...
               <Button className="mt-8 font-black uppercase tracking-widest px-10 h-12 bg-primary text-black hover:bg-primary/95" onClick={() => router.push('/shoecam/capture')}>Open Booth</Button>
            </div>
          )}
        </div>
      </main>
